...
if (!global.__TOPLIMP_COLAB_DB_EVENTS) {
  ...
  colaboradoresDB.on('connected', () => {
    console.log('✅ MongoDB conectado ao banco Colaboradores (cadastro_colaboradores)');
  });
...
const cadastroColaboradorSchema = new mongoose.Schema(
  {
...
cadastroColaboradorSchema.index({ cpf: 1 }, { unique: true });
cadastroColaboradorSchema.index({ arquivado: 1, status: 1, nome: 1 });
cadastroColaboradorSchema.index({ nome: 'text', cpf: 'text', email: 'text' });

module.exports = colaboradoresDB.models.CadastroColaborador || colaboradoresDB.model('CadastroColaborador', cadastroColaboradorSchema, 'cadastro_colaboradores');
